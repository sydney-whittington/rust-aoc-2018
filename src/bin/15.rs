...
impl fmt::Display for Barf {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "barf detected at {:?}", self)
    }
}
...
impl State {
    ...
    pub fn load(input: &str) -> Result<State, Barf> {
    ...
    /// Remove a unit by ID.
    pub fn remove_unit(&mut self, id: UnitId) -> Result<(), Barf> {
        if self.units.remove(&id).is_none() {
            panic!("no such unit: {}", id);
        }

        let pos = match self.position_by_unit.remove(&id) {
            Some(pos) => pos,
            None => panic!("no position for unit: {}", id),
        };

        let old_id = match self.unit_by_position.remove(&pos) {
            Some(id) => id,
            None => panic!("no unit for position: {:?}", pos),
        };

        if old_id != id {
            panic!("wrong unit for position: {:?}", pos);
        }
    ...
    pub fn move_unit(&mut self, from: Pos, to: Pos) -> Result<(), Barf> {
        let id = match self.unit_by_position.remove(&from) {
            Some(id) => id,
            None => panic!("no unit at position: {:?}", from),
        };
    ...
    fn find_unit_position(&self, id: UnitId) -> Result<Pos, Barf> {
    ...
    /// Get unit data.
    pub fn find_unit(&self, id: UnitId) -> Result<&Unit, Barf> {
        match self.units.get(&id) {
            None => panic!("no unit by id: {}", id),
            Some(unit) => Ok(unit),
        }
    }

    pub fn find_mut_unit(&mut self, id: UnitId) -> Result<&mut Unit, Barf> {
    ...
    fn find_attack_target(&self, my_id: UnitId) -> Result<Option<UnitId>, Barf> {
    ...
    pub fn simulate(&mut self) -> Result<u64, Barf> {
        ...
        for tick in 0u64.. {
            ...
            //     write!(out, "{}[2J", char::from_u32(27).unwrap());
            // }
...
fn save_the_elves(mut original: State) -> Result<u64, Barf> {
    original.sleep = 20;
...
pub fn part_one(input: &str) -> Option<u64> {
    Some(State::load(input).unwrap().simulate().unwrap())
}
